 ...
 */
enum childless_t {
	/** Accept childless IKE_SAs as responder, but initiate regular IKE_SAs */
	CHILDLESS_ACCEPT,
	/** Don't accept childless IKE_SAs as responder, don't initiate them */
	CHILDLESS_NEVER,
	/** Try to initiate childless IKE_SAs, but accept regular IKE_SAs */
	/** FIXME: this seems only useful for migration purposes, so maybe not necessary */
	CHILDLESS_PREFER,
