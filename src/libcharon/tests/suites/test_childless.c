...
/**
 * The responder does not support childless IKE_SAs, so the initiator falls
 * back to creating the initial CHILD_SA during IKE_AUTH.
 */
START_TEST(test_fallback)
{
	exchange_test_sa_conf_t conf = {
		.initiator = {
			.childless = CHILDLESS_PREFER,
		},
		.responder = {
...
Suite *childless_suite_create()
{
	...
	tcase_add_test(tc, test_fallback);
	suite_add_tcase(s, tc);
